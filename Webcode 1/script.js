...
function initialization() {
    ...
    let paraElement = newElement("p");
    paraElement.innerText = "The MakeUp API implementation for First Web code ";

    document.body.append(h1Element,paraElement);
...
function renderLayoutWithItems(dataItems){
    ...
    let tableColDiv = document.createElement("div");
      ...
      initialDataItems.forEach((obj) => {
          ...
          //<a href=${mockDataSlice[i].product_link}><image src=${mockDataSlice[i].api_featured_image} alt=${mockDataSlice[i].name}/></a><br/><a href=${mockDataSlice[i].product_link}>More info</a></td><td>${mockDataSlice[i].name}

          let tdElement1 = document.createElement("td");
          tdElement1.innerHTML = `<a href=${obj.product_link} target="_blank"><image src=${obj.api_featured_image} alt=${obj.name}/></a><br/><a href=${obj.product_link}  target="_blank">More info</a>`;
          ...
          //["Product Image","Name","Brand","Description","Price","More Info",]
          let tdElement4 = document.createElement("td");
          tdElement4.innerText = obj.description;

          let tdElement5 = document.createElement("td");
          tdElement5.innerText = resolvePrice(obj);
...
//A function to load data when the pagination elements are clicked.
let clickHandler = function loadTableData(event) {
   ...
   var mockDataSlice = JSON_DATA.slice((pageNumber - 1) * pageSize, pageNumber * pageSize);
      ...
      for (let i = 0; i < mockDataSlice.length; i++) {
          ...
          txt = txt + `<tr><td><a href=${mockDataSlice[i].product_link}  target="_blank"><image src=${mockDataSlice[i].api_featured_image} alt=${mockDataSlice[i].name}/></a><br/><a href=${mockDataSlice[i].product_link}  target="_blank">More info</a></td><td><span class="name">${mockDataSlice[i].name}</span></td>
                       <td>`+resolveBrandname(mockDataSlice[i])+`</td>`
                       +`<td>${mockDataSlice[i].description}</td><td>`+resolvePrice(mockDataSlice[i])+`</td></tr>`;
  ...
  // A function to calculate the clickable anchor items of pagination.
...
(function () {
    ...
    //Fetch the Makeup Items from the API endpoint.
     fetchMakeupItems().then((makeupItems) => {
